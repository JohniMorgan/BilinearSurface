...
public class RotationTransformer {
    ...
    static final public Point3D transformRotateY(Point3D point, Double arg) {
        Double realArg = Math.PI/180*arg; //Угол в радианы...
        Double [][] rotateYMatrix = { //Матрица поворота вдоль оси Y
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                rotated[i] += rotateYMatrix[i][j]*vector[j];
            }
        }
    ...
    //Поворот вдоль оси Z
    static final public Point3D transformRotateZ(Point3D point, Double arg) {
        Double realArg = Math.PI/180*arg; //Угол в радианы...
        Double [][] rotateZMatrix = { //Матрица поворота вдоль оси Z
                {Math.cos(realArg), -Math.sin(realArg), 0.0},
                {Math.sin(realArg), Math.cos(realArg), 0.0},
                {0.0,0.0 , 1.0}
        };
        Double[] vector = {point.getX(), point.getY(), point.getZ()};
        Double[] rotated = {0.0, 0.0, 0.0};
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                rotated[i] += rotateZMatrix[i][j]*vector[j];
            }
        }
        //Получаем новую повёрнутую точку.
        return new Point3D(rotated[0], rotated[1], rotated[2]);
